...
out_dir = "output/jia_grf_"+expf_name+"_"+str(gradCut)+"_"  # for plots
...
#np.savetxt('output/inputkk.csv',np.vstack([lcents,mpibox.stats['ipower']['mean']]).T)
...
if rank==0:
    ...
    np.savetxt('output/ell_recon_raw_autoN0subbed_jia_grf.csv',np.vstack([lcents,cstats['mean'],astats['mean'],rstats['mean']]).T)
