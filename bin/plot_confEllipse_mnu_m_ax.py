...
labels = {'H0':'$H_0$','ombh2':'$\Omega_b h^2$','omch2':'$\Omega_c h^2$','ns':'$n_s$','As':'$A_s$','tau':'$\\tau$','mnu':'$\Sigma m_{\\nu}$ (eV)','nnu':'$N_{eff}$','r':'$r$','m_ax':'$m_{\\rm FDM}$'}
CL = {1:'68%',2:'95%',3:'99%'}
fontsize = 18

dataFile1 = 'data/Oct23_highAcc_FDM_S4cosmo_unlensed_axion_CMB_noise0.05_confEllipse_mnu_m_ax_1sigma.csv'
...
for i in range(len(noises)):
    ...
    dataFile = 'data/Oct23_highAcc_FDM_S4cosmo_unlensed_axion_CMB_noise'+str(noises[i])+'_confEllipse_mnu_m_ax_1sigma.csv'
    #dataLabel = dataLabels[i]
    dataLabel = str(noises[i])+' $\mu$K-arcmin'
    print dataFile
    ...
    ax.set_xlabel(labels[param1],fontsize=22)
    ax.set_ylabel(labels[param2],fontsize=26)
